...
function toggle_human_gif ( hide ) {
	if ( $ ( '.human-loading-gif-wrapper' ).length<1 ) {
		$ ( 'body' ).prepend ( '<div class="human-loading-gif-wrapper"> <div class="human-loading-gif"></div></div>' );
	}
	if ( hide ) {
		$ ( '.human-loading-gif-wrapper' ).fadeOut ();
	} else {
		$ ( '.human-loading-gif-wrapper' ).css ( 'display', 'table' );
	}
}
