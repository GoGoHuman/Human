...
function human_comment_looper ( $key, $val, $attr, $user_metas_simple, $archive_layout, $comment_form_metas, $date_format, $post_id, $user_api_meta, $human_comment_form_metas, $human_looped_template, $user, $comment_user ) {
            ...
            foreach ( $comment_form_metas as $c_meta ) {
                        ...
                        $itemPropWrap = array (
                                    '<div class="comment-wrapper" id="' . $val->comment_ID . '">' );
                        if ( isset ( $val->$c_meta ) && ! empty ( $val->$c_meta ) && $c_meta !== 'thumbs_up' && $c_meta !== 'thumbs_down' && $c_meta != 'comment_flag' ) {
                                    if ( $c_meta === 'comment_date' ) {
                                                $comment_form_metas_to[] = '<time datetime="' . date ( 'c', strtotime ( $val->$c_meta ) ) . '">' . date ( $date_format, strtotime ( $val->$c_meta ) ) . '</time>';
                                    }
                                    elseif ( $c_meta === 'comment_content' ) {
                                                $comment_form_metas_to[] = '<div>' . $val->$c_meta . '</div>';
                                    }
                                    else {
                                    ...
                                    // print_r ( get_comment_meta ( $val->comment_ID ) );
                                    if ( get_comment_meta ( $val->comment_ID, $c_meta ) ) {
                                                if ( $c_meta === 'rating' ) {

                                                            $rating = get_comment_meta ( $val->comment_ID, 'rating' )[ 0 ];
                                                            $comment_form_metas_to[] = '
                                                                                                         <div class="human-star-rating" title="' . __ ( "Rated", "human" ) . ' ' . $rating . ' ' . __ ( "out of", "human" ) . ' 5">' . human_star_rating ( array (
                                                                                    'rated' => $rating ) ) . '<div  class="itemProp">' . $rating . '</div></div>';

                                                            $itemPropWrap[ 0 ] = '<div  id="' . $val->comment_ID . '"  class="comment-wrapper">';
                                                }
            ...
            $archive_l = $itemPropWrap[ 0 ] . $archive_layout . '</div>' . $comment_children_wrap;

            $archive_layout1 = str_replace ( $user_api_meta, $metas, $archive_l );
