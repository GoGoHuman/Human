...
class wpdbBackup {
            ...
            function wpdbBackup () {
                        ...
                        add_action ( 'wp_ajax_save_backup_time', array (
                                    &$this,
                                    'save_backup_time' ) );
                        ...
                        add_action ( 'wp_db_backup_cron', array (
                                    &$this,
                                    'cron_backup' ) );
                        add_action ( 'wp_cron_daily', array (
                                    &$this,
                                    'wp_cron_daily' ) );
                        add_filter ( 'cron_schedules', array (
                                    &$this,
                                    'add_sched_options' ) );
                        add_filter ( 'wp_db_b_schedule_choices', array (
                                    &$this,
                                    'schedule_choices' ) );
            ...
            function set_page_url () {
                        ...
                        if ( function_exists ( 'wp_create_nonce' ) )
                                    $query_args = array_merge ( $query_args, array (
                                                '_wpnonce' => wp_create_nonce ( $this->referer_check_key ) ) );
            ...
            function update_notice_action () {
                        ...
                        if (
                                    (
                                    isset ( $_REQUEST[ 'action' ] ) && ('upgrade-core' == $_REQUEST[ 'action' ])
                                    ) || (
                        ...
                        if ( is_writable ( $this->backup_dir ) ) {
                                    ...
                                    if ( ! $this->fp ) {
                                                ...
                                                $this->error ( array (
                                                            'loc' => 'frame',
                                                            'kind' => 'fatal',
                                                            'msg' => __ ( 'The backup file could not be saved.  Please check the permissions for writing to your backup directory and try again.', 'wp-db-backup' ) ) );
                                    }
                                    else {
                                                if ( $table == '' ) {
                                    ...
                                    $this->error ( array (
                                                'kind' => 'fatal',
                                                'loc' => 'frame',
                                                'msg' => __ ( 'The backup directory is not writeable!  Please check the permissions for writing to your backup directory and try again.', 'wp-db-backup' ) ) );
                        }
                        if ( $this->fp )
                                    $this->close ( $this->fp );
		...
		';
            ...
            function perform_backup ( $dest ) {
                        ...
                        foreach ( $results as $index => $value ) {
                                    foreach ( $value as $tableName ) {
                                                if ( $table_prefix . 'users' !== $tableName || $table_prefix . 'usersmeta' !== $tableName ) {
                                                            $also_backup[] = $tableName;
                                                }
                                    }
                        }

                        $this->backup_file = $this->db_backup ( $core_tables = [ ], $also_backup, $dest );
            ...
            function admin_header () {
                        ...
                        <?php if ( function_exists ( 'wp_schedule_event' ) ) : // needs to be at least WP 2.1 for ajax                 ?>
                                    ...
                                    ?> ) ) {
                                    					    ...
                                    					    backupTime.ondblclick = function ( e ) {
                                    						    span.parentNode.removeChild ( span );
                                    						    clickTime ( e, backupTime );
                                    					    };
                                    			    ...
                                    			    var clickTime = function ( e, backupTime ) {
                                    				    ...
                                    				    if ( !!saveTButton )
                                    					    saveTButton.onclick = function ( e ) {
            ...
            function admin_menu () {
                        $_page_hook = add_management_page ( __ ( 'Backup', 'wp-db-backup' ), __ ( 'Backup', 'wp-db-backup' ), 'import', $this->basename, array (
                                    &$this,
                                    'backup_menu' ) );
                        add_action ( 'load-' . $_page_hook, array (
                                    &$this,
                                    'admin_load' ) );
            ...
            function fragment_menu () {
                        $page_hook = add_management_page ( __ ( 'Backup', 'wp-db-backup' ), __ ( 'Backup', 'wp-db-backup' ), 'import', $this->basename, array (
                                    &$this,
                                    'build_backup_script' ) );
            ...
            function backup_table ( $table, $segment = 'none' ) {
                        ...
                        if ( ($segment == 'none') || ($segment >= 0) ) {
                                    ...
                                    do {
                                                ...
                                                $where = '';
                                                if ( isset ( $excs[ 'spam' ] ) && isset ( $excs[ 'revisions' ] ) ) {
...
// end backup_table()
            ...
            function db_backup ( $core_tables, $other_tables, $dest ) {
                        global $table_prefix, $wpdb;
                        mkdir ( $dest );
                        if ( is_writable ( $dest ) ) {
                                    $this->fp = $this->open ( $dest . '/' . $this->backup_filename );
                                    if ( ! $this->fp ) {
                                                $this->error ( __ ( 'Could not open the backup file for writing!', 'wp-db-backup' ) );
                                                return false;
                                    }
...
//wp_db_backup
            ...
            function send_mail ( $to, $subject, $message, $diskfile ) {
                        ...
                        if ( is_object ( $phpmailer ) && ( strtolower ( get_class ( $phpmailer ) ) == 'phpmailer' ) ) {
                                    ...
                                    do_action_ref_array ( 'phpmailer_init', array (
                                                &$phpmailer ) );
                        ...
                        if ( 'http' == $delivery ) {
                                    if ( ! file_exists ( $diskfile ) ) {
                                                if ( empty ( $_GET[ 'download-retry' ] ) ) {
                                                            $this->error ( array (
                                                                        'kind' => 'fatal',
                                                                        'msg' => sprintf ( __ ( 'File not found:%s', 'wp-db-backup' ), "&nbsp;<strong>$filename</strong><br />" ) . '<br /><a href="' . $this->page_url . '">' . __ ( 'Return to Backup', 'wp-db-backup' ) . '</a>' ) );
                                                }
                                                else {
                                                            return true;
                                                }
                                  ...
                                  <form method="post" action="">
                                            ...
                                            <fieldset class="options">
                                                      ...
                                                      if ( $cron_old || $cron ) :
                                                                  ...
                                                                  elseif ( $cron_old ) :
                                                                              ?><p><?php printf ( __ ( 'Last WP-Cron Daily Execution: %s', 'wp-db-backup' ), gmdate ( $datetime, get_option ( 'wp_cron_daily_lastrun' ) + (get_option ( 'gmt_offset' ) * 3600) ) ); ?><br /><?php printf ( __ ( 'Next WP-Cron Daily Execution: %s', 'wp-db-backup' ), gmdate ( $datetime, (get_option ( 'wp_cron_daily_lastrun' ) + (get_option ( 'gmt_offset' ) * 3600) + 86400 ) ) ); ?></p><?php
                                                                  endif;
                                                                  ?><form method="post" action="">
                                                                  <?php if ( function_exists ( 'wp_nonce_field' ) ) wp_nonce_field ( $this->referer_check_key ); ?>
...
// end wp_backup_menu()
                                          ...
                                          function schedule_choices ( $schedule ) { // create the cron menu based on the schedule
                                                      ...
                                                      $schedule = array_merge ( array (
                                                                  'never' => array (
                                                                              'interval' => 0,
                                                                              'display' => __ ( 'Never', 'wp-db-backup' ) ) ), ( array ) $schedule_sorted );
                                          ...
                                          function can_user_backup ( $loc = 'main' ) {
                                                      ...
                                                      if ( false == $can )
                                                                  $this->error ( array (
                                                                              'loc' => $loc,
                                                                              'kind' => 'fatal',
                                                                              'msg' => __ ( 'You are not allowed to perform backups.', 'wp-db-backup' ) ) );
                                                      return $can;
                                          }
                                          ...
                                          function verify_nonce ( $rec = '', $nonce = 'X', $loc = 'main' ) {
                                                      if ( wp_verify_nonce ( $rec, $nonce ) )
                                                                  ...
                                                                  $this->error ( array (
                                                                              'loc' => $loc,
                                                                              'kind' => 'fatal',
                                                                              'msg' => sprintf ( __ ( 'There appears to be an unauthorized attempt from this site to access your database located at %1s.  The attempt has been halted.', 'wp-db-backup' ), get_option ( 'home' ) ) ) );
                                          }
                                          ...
                                          function validate_file ( $file ) {
                                                      if ( (false !== strpos ( $file, '..' )) || (false !== strpos ( $file, './' )) || (':' == substr ( $file, 1, 1 )) )
                                                                  $this->error ( array (
                                                                              'kind' => 'fatal',
                                                                              'loc' => 'frame',
                                                                              'msg' => __ ( "Cheatin' uh ?", 'wp-db-backup' ) ) );
                                          }
