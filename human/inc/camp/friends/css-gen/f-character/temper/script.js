...
//  console.log($(this));
	...
	$ ( '#human-resolution-slider span' ).prepend ( '<span id="res-slider-caption">'+res_max_width+'</span>' );
	...
	$ ( "#human_screen_res" ).on ( 'click', function () {
		$ ( this ).toggleClass ( 'ignore-res' );
		css_select_links ();
		$ ( this ).blur ();
	} );
	...
	$ ( '#human_new_section_action' ).on ( 'click', function ( e ) {
		...
		var data = {
			action: action,
			nonce: humanAjax.nonce,
			new_section: "", // depriciated since 2.0 $ ( '#human_new_section' ).val (),
	...
	$ ( '#template_wraps' ).live ( 'change', function ( e ) {

		$ ( '.data-class' ).fadeOut ( );
		$ ( 'tr.related-tag-row' ).fadeOut ();
		if ( $ ( '#template_wraps' ).val ().length>0&&$ ( '#template_wraps' ).val ()!==0 ) {
			var option = $ ( '#custom_sections' ).find ( 'option[data-class="'+$ ( this ).val ()+'"]' );
			option.fadeIn ( );
			$ ( 'tr[data-class=".human-page '+$ ( this ).val ()+'"]' ).fadeIn ();

		} else {
			//alert ( 'empty folder' );
			$ ( '#selected_tag' ).val ( '.human-page' );
			$ ( 'tr[data-class="default"]' ).fadeIn ();
		}
	} );
	$ ( '#custom_sections' ).live ( 'change', function () {
		$ ( 'tr.related-tag-row' ).fadeOut ();

		if ( $ ( '#custom_sections' ).val ().length>0&&$ ( '#custom_sections' ).val ()!==0 ) {
			var tr = 'tr[data-class-section=".human-page '+$ ( this ).val ()+'"]';

			$ ( tr ).fadeIn ();
		} else if ( $ ( '#template_wraps' ).val ().length>0&&$ ( '#template_wraps' ).val ()!==0 ) {
			$ ( 'tr[data-class=".human-page '+$ ( '#template_wraps' ).val ()+'"]' ).fadeIn ();
		} else {
