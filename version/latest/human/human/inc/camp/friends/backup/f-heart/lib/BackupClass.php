...
//Execute the dump command
                                                            ...
                                                            if ( strtoupper ( substr ( PHP_OS, 0, 3 ) ) === 'WIN' ) {
                                                                        ...
                                                                        if ( get_option ( 'human-mysqldump-path' ) ) {
                                                                                    ...
                                                                                    return 'You are using Windows mashine - please specify full path to mysql';
                                                                        }
                                                            }
                                                            if ( ! empty ( $this->databaseVars[ 'password' ] ) ) {
                                                                        $db_pass = ' -p' . $this->databaseVars[ 'password' ];
                                                            }
                                                            else {
                                                                        $db_pass = '';
                                                            }
...
//Transfer the compressed file to Amazon S3 storage (If asked to)
             ...
             *
             */
            public function executeRestore ( $backupPath ) {
//Fetch all local backup files available

                        $dumpedArchives = glob ( $backupPath );
...
//Restore all files available in the current folder
                                    ...
                                    system ( $mysqlpath . " -u " . $this->databaseVars[ 'login' ] . $db_pass . " " . $this->databaseVars[ 'database_name' ] . " < " . $backupPath );
                                    return "<br>" . $backupPath . " mysql table imported";
                        }
                        else {
...
//if directory doesn't
//exist attempt to create it after checking the $force_create param
            ...
            private function createDir ( $directory_path ) {
                        if ( mkdir ( $directory_path, 0777, true ) ) {
                                    echo 'DB directory has been created<br>';
                                    return true;
                        }
                        else {
...
//     mkdir ( $this->backupDir );
                        $this->folderName = $this->backupDir;
            }
