...
function copy_directory ( $source1, $dest1, $permissions = 0777 ) {
            // Check for symlinks
...
<div class="wrap human_wrapper" id="human_setting_wrapper">
          ...
          if ( isset ( $_POST[ 'template_name' ] ) ) {
                      ...
                      if ( function_exists ( 'backup_tables' ) ) {
                                  ...
                                  $min_css = $directory . $template_name . '/wp-content/css-mins.html';
                                  $human_css = $directory . $template_name . '/wp-content/human.css';
                                  //$html_css = $directory . $template_name . '/html_css.html';
                                  $custom_css = $directory . $template_name . '/wp-content/custom_css.html';
                                  $custom_fonts = $directory . $template_name . '/wp-content/custom_fonts.html';
                                  $child_css = $directory . $template_name . '/wp-content/child-style.css';
                                  $file = fopen ( $min_css, 'w+' );

                                  fwrite ( $file, get_option ( 'STYLE-GENS-MINIFIED' ) );
                                  ...
                                  fwrite ( $file, html_entity_decode ( stripslashes ( trim ( get_option ( 'HUMAN_CUSTOM_CSS' ) ) ) ) );

                                  fclose ( $file );

                                  $file = fopen ( $custom_fonts, 'w+' );

                                  fwrite ( $file, html_entity_decode ( stripslashes ( trim ( get_option ( 'HUMAN-STYLE-GENS-FONTS' ) ) ) ) );
                                  ...
                                  foreach ( $common_wp_content_items as $item ) {
                                              ...
                                              $rootpath = $wp_content_path . $item . '/'; // file filter, you could specify a extension using *.ext
                                              ob_start ();
          ...
          if ( isset ( $template_created ) ) {
                      echo '<h4 style="color:blue">Baby Created</h4>';
                      ob_end_flush ();
          }
          ?>
          <form method="post">
